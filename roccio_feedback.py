...
def roccio_feedback():
    file = "cosine_similarity_rocchio"
    system_name = "ROCCHIO_COSINE_SIM"
    dict_idf = store_tfidfs(vocabulary)
    create_doc_query_vectors()
    read_cosine_similarity_results()
    for query_id,query_text in queries.items():
        expand=expand_query(query_id)
        expanded_query=query_text+expand
        print query_id+":"+query_text+"---"+expand+"\n"
