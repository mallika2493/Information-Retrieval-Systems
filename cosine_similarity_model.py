...
def counters_all_docs():
    ...
    for filename in filenames:
        ...
        fileCon = fd.read()
        fd.close()
        counter_docs[filename[:-4]] = Counter(fileCon.strip().split())
    return counter_docs
...
def write_simalirity_matrix(query_id,sorted_consine_simularity,file,system_name):
    ...
    with open(dest_folder+file+".csv", 'a') as csv_file:
        ...
        for tuple in sorted_consine_simularity:
            ...
            writer_tf.writerow([query_id,"Q0",tuple[0], limit, tuple[1],system_name])
...
def calculate_doc_magnitude(filename,dict_tfidf):
    doc_mag = 0
    counters=all_counters[filename[:-4]]
    for term in counters:
        if term in dict_tfidf:
            doc_mag += pow(counters[term]*dict_tfidf[term], 2)
    return doc_mag

def compute_cosine_similarity(query_id,query,dict_idf,file,system_name):
    ...
    for filename in filenames:
        ...
        for term in query_tf:
            if term not in counters:
                i = i + 1
                if i==len(query_tf):
                    cosine_simularity[filename[:-4]] = 0
                    flag=1
                continue
            else:
        ...
        if flag == 0:
            ...
            cosine_simularity[filename[:-4]]=similarity
    write_simalirity_matrix(query_id,sorted(cosine_simularity.items(),key=lambda t:t[1],reverse=True),file,system_name)
...
def main():
    ...
    for key in queries:
        query_id=key
        compute_cosine_similarity(query_id.strip(),queries[key],dict_idf,file,system_name)
